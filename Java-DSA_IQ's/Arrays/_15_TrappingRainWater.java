public class _15_TrappingRainWater {
    ...
    /*
     * Approach:
     * 1. Create a function trappingRainWater that takes an array of heights as
     * input.
     * 2. Initialize two arrays leftMax and rightMax to store the maximum heights to
     * the left and right of each bar.
     * 3. LeftMax[0] is initialized to the height of 1st bar. Ex: leftMax[0] =
     * height[0].
     * 4. Apply loop to fill leftMax array. from i = 1 to i<n, i++; LeftMax[i] =
     * max(height[i], leftMax[i-1]).
     * 5. RightMax[n-1] is initialized to the height of last bar. Ex: rightMax[n-1]
     * = height[n-1].
     * 6. Apply loop to fill rightMax array. from i = n-2 to i>=0, i--; RightMax[i]
     * = max(height[i], rightMax[i+1]).
     * 7. Initialize a variable trappedWater to 0 to store the total trapped water.
     * 8. Apply loop to calculate water trapped at each bar. from i = 0 to i<n, i++;
     * waterAtCurrentBar = min(leftMax[i], rightMax[i]) - height[i].
     * 9. If waterAtCurrentBar > 0, add it to trappedWater. Ex: trappedWater +=
     * waterAtCurrentBar.
     * 10. Return trappedWater as the result.
     * 11. The time complexity of this approach is O(N) and the space complexity is
     * O(N).
     * 12. The space complexity can be reduced to O(1) by using two pointers instead
     * of two arrays.
     */
